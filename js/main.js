$(document).ready(
  function () {
    ...
    }; //creandola come valore funzione la posso reimpostare dinamicamente al bisogno
    ...
    function getAnswer(input) {
      ...
      const replies = [
        ...
        ["Ciao ciao", "A presto!"],
        /*8 hello world */
        ["Hello world? Ma che ti credi di parlare con una macchina?!"]
      ];
      ...
      for (let x = 0; x < triggers.length; x++) {
        for (let y = 0; y < triggers[x].length; y++) {
          if (input.length >= 3 && triggers[x][y].includes(input)) {
            var items = replies[x];
    ...
    function sendMsg() {
      ...
      messageBox().append(template({
        ...
        "msg": msg,
        "time": time
      })); //inietto un div dotato di classi .message e .sent in .messages
